...
#         result_agent_referrer = AiAssistant.agent_referrer(self.df, AiAssistant.llm).invoke(AiAssistant.prompt_agent_referrer(request))
...
#             result_chain_handler = AiAssistant.chain_handler(AiAssistant.llm).invoke({"question": request, "format_records": format_records})
#             return MessageDTO(
...
#         result_chain_except = AiAssistant.chain_except(AiAssistant.llm).invoke({"question": request})
#         return MessageDTO(
...
# pip install chromadb langchain_experimental langchain_huggingface
...
from backend.src.config import settings
from backend.src.core.domain.message import MessageDTO
from backend.src.infrastructure.database.repositories.order import retrieve_pending_order
...
class AiAssistant:
    ...
    async def refresh_df(self):
        async for uow in get_unit_of_work():
            ...
            self.type_meals_weight = type_meals_weight
    ...
    def format_dataframe(self, df_curr):
        formatted_rows = []
    ...
    async def get_answer(self, request: str, user_id: int) -> MessageDTO:
        ...
        async def basket(state: InputState):
            # res = self.chain_basket.invoke({"text": state["question"], "basket_meals": "пицца 300 рублей, шаурма 200 рублей, напиток 123 руб"}).content
            res = self.agent_basket.invoke({"messages": [("system", await retrieve_pending_order(user_id)), ("user", state["question"])]}).get("messages")[-1].content
            return {"answer": res}
